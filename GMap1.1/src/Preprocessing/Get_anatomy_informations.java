...
public class Get_anatomy_informations {
	public static void main(String args[]) throws IOException
	{
		//定义文件的写操作
		BufferedWriter bfw_concepts= null;
		BufferedWriter bfw_labels= null;
		...
		try{
			...
			bfw_labels=new BufferedWriter(new FileWriter(Store_Path+"/labels.txt"));
		...
		String ontPath ="Datasets/anatomy/"+Ontology_name+".owl";//读取本体的路径
		...
		ArrayList<String> Classes_Label=Onto.GetAllConceptLabel();
		//获取本体中所有的对象属性
		...
		Classes_Label=keepLabel(Classes_Label);
		for(int j=0;j<Classes_Label.size();j++)
		{
			bfw_labels.append(Classes_Label.get(j)+"\n");		//读取的时候会默认识别换行符的
		}
		...
		TreeMap_Tools sub=new TreeMap_Tools(Subclasses_Direct);
		System.out.println(sub.getNumberOfMap());
	...
	//}//循环
	public static ArrayList<String> keepLabel(ArrayList<String> ClassLabel)
	{
