...
public class Old_Main_cmd {
	static Sim_Tools tool=new Sim_Tools();
	static BufferedWriter bfw_Result= null;
	//static BufferedWriter bfw_middle= null;
	public static void main(String args[]) throws Exception
	{
		//Ontologies cmt.owl Conference.owl result
		String ontologyPath=args[0];  //path
		String ontologyName1=args[1];  //name of ontology1
		String ontologyName2=args[2];  //name of ontology2
		
		String saveAlignmentPath=args[3]+"/"+args[1].replaceAll(".rdf", "").replaceAll(".owl", "")+"-"+args[2].replaceAll(".rdf", "").replaceAll(".owl", "");
		String saveResultsPath=args[3]+"/"+args[1].replaceAll(".rdf", "").replaceAll(".owl", "")+args[2].replaceAll(".rdf", "").replaceAll(".owl", "")+".txt";
		
		/*String ontologyPath="Ontologies";  //path
		String ontologyName1="cmt.owl";   //name of ontology1
		String ontologyName2="Conference.owl";  //name of ontology2
		
		
		String saveAlignmentPath="result"+"/"+ontologyName1.replaceAll(".rdf", "").replaceAll(".owl", "")+"-"+ontologyName2.replaceAll(".rdf", "").replaceAll(".owl", "");
		String saveResultsPath="result"+"/"+ontologyName1.replaceAll(".rdf", "").replaceAll(".owl", "")+"-"+ontologyName2.replaceAll(".rdf", "").replaceAll(".owl", "")+".txt";*/
		
		//String saveResultsPath="Results/2.txt";		
		//String resultPath2="Results/middle_Result.txt";	
		try
		{			
			bfw_Result=new BufferedWriter(new FileWriter(saveResultsPath));	
		}
		catch(IOException e)
		{
			e.printStackTrace();		
		...
		//Loading
		onto1.readOnto(ontologyPath+"/"+ontologyName1);
		onto2.readOnto(ontologyPath+"/"+ontologyName2);
		...
		/*	for(int i=0;i<classesMap.size();i++)
			...
			for(int i=0;i<classPairSize;i++)
			{
				...
				/*else//如果有词组的比较，主要看编辑距离则会起到一定反面的效果,由于组合来切词的时候会造成一定相似度的压缩
					S0=0.7*Math.max(semanticSimValue, tfidfSimValue);*/
				//S0=0.7*Math.max(Math.max(editSimValue, semanticSimValue),tfidfSimValue);//pooling的方法更合适一些吧
				//S0=Math.min(0.7, Math.max(Math.max(editSimValue, semanticSimValue),tfidfSimValue));//pooling的方法更合适一些吧
				//if(editSimValue==semanticSimValue&&editSimValue>0)//有可能出现很多WordNet的计算错误
				/*if((length1==1||length2==1)&&editSimValue==1)//部分切词的问题如Inbook与InBook
					S0=0.7*editSimValue;
				if(editSimValue==1)//有可能出现很多WordNet的计算错误，因此编辑距离相等时，则两个概念相等
					S0=S0+0.0000001;*/
				...
				//S0=0.7000001;
				//S0=Math.min(0.7,Math.max(Math.max(editSimValue, semanticSimValue),tfidfSimValue));//pooling的方法更合适一些吧
				//S0=(editSimValue+semanticSimValue)/2;
				/**
				...
				if(finalPositive>=0.8)
				{
					System.out.println(pairName+","+finalPositive);
					roughMap.add(pairName+","+finalPositive);
				}
				else if(iteration==0&&finalPositive>=0.6&&finalPositive<0.8)//很多隐藏的点，由于知识不完备（实例未给出，关系未给出），导致其无法被识别
				{
					...
					hiddenClassesMap.add(pairName+","+finalPositive);
				}
				/*ArrayList<String> hiddenClassesMap=new ArrayList<String>();
		...
		ontologyName1=ontologyName1.replaceAll(".rdf", "").replaceAll(".owl", "");
		ontologyName2=ontologyName2.replaceAll(".rdf", "").replaceAll(".owl", "");
		OAEIAlignmentOutput out=new OAEIAlignmentOutput(saveAlignmentPath,ontologyName1,ontologyName2);
		for(int i=0;i<classesMap.size();i++)
		{
			String parts[]=classesMap.get(i).split(",");
